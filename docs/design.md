...
Code blocks display highlighted code without backtick fences or language labels. Older assistant responses end with a muted separator; the latest response does not. HTTP(S) and mail links use native OSC 8 hyperlinks, including wrapped labels and web URLs in tool output. Use the terminal's link-opening gesture (Cmd-click in Ghostty on macOS). Unsafe schemes, control-bearing targets, and targets longer than 4 KiB stay plain text.

The mouse wheel scrolls transcript output in both normal and expanded views without changing the composer draft or prompt history. Reading positions survive streaming updates and resize; reaching the bottom resumes following new output. `Ctrl+L` returns the normal view to the latest output. Mouse reporting uses only button/wheel events, not pointer motion. Hold Shift while dragging to use native terminal text selection; mouse modes are restored on exit.
...
The same gate checks fence-free code, balanced OSC 8 links, wheel navigation without draft changes, narrow resize while browsing, older-response separators, no latest-response separator, and restored mouse reporting modes. Unit regressions cover URL-only target changes/removal, Unicode link widths, oversized or unsafe targets, and incremental cache invalidation.
